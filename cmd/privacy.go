...
// privacyCmd represents the command to find secrets
var privacyCmd = &cobra.Command{
	Use:   "privacy",
	Short: "Find privacy related fields in a directory",
	Long: `Find privacy related items in a directory of files.
	
	Supported privacy items include:
	...
	Run: func(cmd *cobra.Command, args []string) {
		start := time.Now()
		opts := buildPrivacyOptions(cmd, args)
		log.Debugf("Privacy command on: %s", opts.Directory)
		checks := getPrivacyChecks(opts)
...
func getPrivacyChecks(opts options.Options) []check.Check {
	var checks []check.Check
	checks = append(checks, check.GetChecks("checks/privacy.json")...)
	return checks
}

func buildPrivacyOptions(cmd *cobra.Command, args []string) options.Options {
	directory := args[0]
	if directory == "" {
		path, err := os.Getwd()
		if err != nil {
	...
	tag := "privacy" // Always use the secrets tag.
